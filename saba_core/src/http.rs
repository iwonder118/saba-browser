...
impl HttpResponse {
    pub fn new(raw_response: String) -> Result<Self, Error> {
        let preprocessed_reponse = raw_response.trim_start().replace("\r\n", "\n");
        let (status_line, remaining) = match preprocessed_reponse.split_once("\n") {
            Some((s, r)) => (s, r),
            None => {
                return Err(Error::Network(format!(
                    "invalid http response: {}", preprocessed_reponse
                )))
            }
        };
...
mod tests {
    ...
    fn test_status_line_only() {
        ...
        let res = HttpResponse::new(raw).expect("failed to parse httpresponse");
